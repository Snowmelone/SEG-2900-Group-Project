...
export default function TeamGrid(props: any) {
    ...
    return (
        <div className="flex justify-center items-center w-full md:px-16 ">
            ...
            <motion.div key={index} className="flex mx-2 max-w-[300px] w-full mt-2  rounded-lg bg-transparent blurrybg border-2 z-20  items-center justify-center cursor-pointer select-none hover:shadow-lg"
                animate={{ y: 0, opacity: 1 }}
                initial={{ y: 100, opacity: 0 }}
                transition={{ duration: 0.5, delay: item.delay }}>
                <div className="h-[400px] md:h-[500px] max-w-[300px] w-full flex flex-col items-center justify-between"> {/* Adjusted height */}
                    <div className="relative w-full h-3/4 rounded-md flex items-center justify-center">
                        <img
                            src={item.image}
                            alt='s'
                            className='object-cover max-w-[300px] w-full  h-full rounded-md' 
                        />
                    </div>
                    <div className='z-30 flex flex-col justify-end h-full'>
                        <Card className='py-3 max-w-[300px] h-full rounded-t-none'>
